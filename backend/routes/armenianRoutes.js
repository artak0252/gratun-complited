...
router.get('/', async (req, res) => {
          try {
                    ...
                    const items = await Armenian.find(filter).sort({ createdAt: -1 });
...
router.put('/:id', adminOnly, validateObjectId, (req, res, next) => {
          ...
          try {
                    ...
                    const updateData = { title, category, author, excerpt, content };
                    ...
                    const updatedItem = await Armenian.findByIdAndUpdate(req.params.id, updateData, { new: true, runValidators: true });
...
router.delete('/:id', adminOnly, validateObjectId, async (req, res) => {
          try {
                    const item = await Armenian.findByIdAndDelete(req.params.id);
                    if (!item) return res.status(404).json({ message: 'Նյութը չգտնվեց' });
